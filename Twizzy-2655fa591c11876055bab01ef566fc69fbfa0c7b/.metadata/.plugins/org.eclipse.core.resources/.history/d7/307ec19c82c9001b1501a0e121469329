...
public class MaBibliothequeTraitementImage {
	...
	public static Mat seuillage(Mat input, int seuilRougeOrange, int seuilRougeViolet,int seuilSaturation){
		...
		Mat seuil_rouge_droit = new Mat();
		Mat seuil_rouge_gauche = new Mat();
		Mat seuil_saturation = new Mat();
		Mat destination= new Mat();
		//Comparaison et saturation des pixels dont la composante rouge est plus grande que le seuil rougeViolet
		Core.compare(channels.get(0),rougeviolet,seuil_rouge_droit,Core.CMP_GT);
		Core.compare(channels.get(0),rougeorange,seuil_rouge_gauche,Core.CMP_GT);
		Core.bitwise_not(seuil_rouge_gauche, seuil_rouge_gauche);
		Core.compare(channels.get(1), saturation, seuil_saturation,Core.CMP_GT);
		Core.bitwise_not(destination, destination);
		Core.bitwise_and(seuil_rouge_droit,seuil_saturation, destination);
		Core.bitwise_and(seuil_rouge_gauche,seuil_saturation, seuil_rouge_gauche);
		Core.bitwise_or(seuil_rouge_gauche,destination, destination);
